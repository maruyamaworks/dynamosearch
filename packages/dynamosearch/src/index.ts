...
class DynamoSearch {
  ...
  async search(query: string, options: { attributes?: string[] } = {}) {
    ...
    for (let i = 0; i < attributes.length; i++) {
      ...
      for (let j = 0; j < words.length; j++) {
        ...
        Items?.forEach((item) => {
          ...
          candidates.set(JSON.stringify(keys), (candidates.get(JSON.stringify(keys)) ?? 0) + parseInt(tf, 16) * (attributes[i].boost ?? 1));
        });
      }
    }
